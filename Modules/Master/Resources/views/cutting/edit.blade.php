    ...
    <section id="basic-input">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Edit Cutting Ratios</h4>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                @endif
                <form class="table-responsive" action="{{route('master.cutting.update', ['id'=>$ratios->id])}}"
                    method="post">
                    @csrf
                    <div class="col-md-12 col-12 mb-3">
                        <label for="validationTooltip01">Name</label>
                        <input type="text" class="form-control" name="name" placeholder="Name" value="{{$ratios->name}}"
                    ...
                    <table class="title-form">
                        <tbody>
                            ...
                            <tr>
                                @php
                                $child = $ratios->child->pluck( "ratio", "size_id");
                                @endphp

                                @foreach($sizes as $size)
                                <td><input type="number" onchange='updateTotal();' id="cutting_ratios{{$size->id}}"
                                        value="{{ isset($child[$size->id])? $child[$size->id]: "" }}"
                                        name="cutting_ratios[{{$size->id}}]" class="form-control input">
                                    @endforeach</td>


                                <td><input type="number" readonly="" id="total" name="sum" class="form-control">
                                </td>
                                @error('sum')
