   ...
   <section id="basic-input">
         ...
         <div class="col-md-12">
            <div class="card">
               <div class="card-header">
                  <h4 class="card-title">Plan Name : @{{cutting_plan_no}}</h4>
      ...
      <div class="row">
         <div class="col-xl-2 col-md-2 col-sm-6 col-6">
                  <a href="javascript:void(0)" class="{{$cutting_plan->status  == 0 ? 'btn btn-sm btn-danger' : 'btn btn-sm btn-success'}}">{{$cutting_plan->status  == 0 ? "Pending" : "Processed"}}</a>
         </div>
      </div>
                  ...
                  <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0" style="">
                     <thead class="">
                        <tr style="background-color: rgb(50, 173, 137) !important; color: white !impotant;">
                           ...
                           @endforeach
                           <th >Total Qty</th>
                           @if($cutting_plan->status  == 0)<th>Action</th>@endif
                        </tr>
                     </thead>
                     <tbody>
      ...
      <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
          <div class="modal-content">
              ...
              <div class="modal-body">
               <form class="table-responsive" action="{{route('update.planAttribute', ['id' => $detail->id]) }}" method="post">
                  @csrf
...
var app = new Vue({
           ...
           data: {
                 sizes:@json($sizes),
                 cutting_plan:@json($cutting_plan),
                 cutting_plan_no:@json($cutting_plan->plan_season_code),
                 child:"",
                 },
           mounted(){
           ...
           methods: {
                 push_to_child(){
                    this.child = @json($cutting_plan['detail']);
                 }
                     },
           computed: {
                      },
