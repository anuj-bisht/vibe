...
<div class="container">
    ...
    <section id="collapsible">
                                            ...
                                            <div class="col-md-3 col-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control " id="main_category" name="main_category" onchange="getCategory(this)" data-index="9" onkeydown="setNextField(event)">
                                                       ...
                                                       <option value={{$mainCategory->id}} {{ $main_category_selected == $mainCategory->id ? "selected":"" }}>{{$mainCategory->name}}</option>
                                                       @endforeach
                                                    </select>
                                                    @error('main_category')
        ...
        <?php
        if($sub_category_selected !=  null){
            $category = Modules\Master\Entities\Category::whereIn('main_category', [$main_category_selected])->get();
        }
         ?>
                                       @php
                                            ...
                                            <div class="col-md-3 col-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control"  id="category" multiple="multiple" name="category[]"  onchange="getSubCategory(this)" data-index="10" onkeydown="setNextField(event)">
                                                        @if($sub_category_selected == null)
                                                        @else
                                                         {{-- @foreach($sub_category_selected as $sc_id) --}}
                                                         @foreach($category as $Category)
                                                        {{-- <option value={{$sc_id}} {{ $sc_id == $Category->id ? "selected":"" }}>
                                                            {{$Category->name}}</option> --}}
                                                        <option value="{{ $Category->id }}" @if(in_array($Category->id,  $sub_category_selected ))  selected @endif >{{ $Category->name }}</option>   
                                                         @endforeach
                                                        {{-- @endforeach  --}}
    ...
    <section id="card-demo-example">
        <div class="row match-height">
            ...
            <div class="col-md-6 col-lg-3 " onclick="showqty({{$data['id']}})">
                <div class="card">
                    <img class="card-img-top" src="{{asset('ProductImage/'.$data['p_image1'])}}" alt="Card image cap" />
